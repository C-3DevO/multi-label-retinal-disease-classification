...
def plot_loss_curves(train_losses, val_losses, backbone, mode, save_dir="plots"):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title(f"Training and Validation Loss {backbone} with Cross_Balanced Loss")
...
if __name__ == "__main__":
    ...
    train_one_backbone(backbone, train_csv, val_csv, test_csv, train_image_dir, val_image_dir, test_image_dir,
                           epochs=25, batch_size=32, lr=1e-5, img_size=256, pretrained_backbone=pretrained_backbone,
                           mode="full_finetune",loss_type="class_balanced")
