...
def find_best_threshold(model, val_loader, device):
    ...
    for thr in np.linspace(0.1, 0.9, 17):
        ...
        if macro_f1 > best_f1:
            best_f1, best_thr = macro_f1, thr

    print(f"Best validation threshold: {best_thr:.3f}, Macro F1: {best_f1:.4f}")
    return best_thr
...
def plot_loss_curves(train_losses, val_losses, backbone, mode, save_dir="plots"):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title(f"Training and Validation Loss ({backbone}, {mode})")
...
def train_one_backbone(backbone, train_csv, val_csv, test_csv, train_image_dir, val_image_dir, test_image_dir, 
                       epochs=10, batch_size=32, lr=1e-4, img_size=256, save_dir="checkpoints",pretrained_backbone=None,mode="freeze_backbone"):
    ...
    pos_weights = get_class_weights(train_csv).to(device)
...
if __name__ == "__main__":
    ...
    train_one_backbone(backbone, train_csv, val_csv, test_csv, train_image_dir, val_image_dir, test_image_dir,
                           epochs=25, batch_size=32, lr=3e-4, img_size=256, pretrained_backbone=pretrained_backbone,mode="full_finetune")
